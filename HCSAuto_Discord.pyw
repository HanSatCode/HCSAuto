...
srcPath = srcPath.replace("HCSAuto_Discord.pyw", "")
...
try : 
    f = open(str(srcPath)+"src/bot.csv",'r',encoding='utf-8-sig')
...
try : 
    f = open(str(srcPath)+"src/member.csv",'r',encoding='utf-8-sig')
except :
    @bot.event
    async def on_ready() :
        channel = bot.get_channel(int(''.join(discord_Set[0][0])))
...
async def on_ready() :
    ...
    try :
        for x in range (0, len(database)) :
            ...
            driver = webdriver.Chrome(str(srcPath) + "src/chromedriver.exe", options=options)
